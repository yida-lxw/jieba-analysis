...
import com.huaban.analysis.jieba.dic.DictSegment;
import com.huaban.analysis.jieba.dic.WordDictionary;
...
public class JiebaSegmenter {
    ...
    public JiebaSegmenter(String mode) {
        this.config = DefaultConfiguration.getInstance();
        this.mode = mode;
    }
    ...
    private SegMode toMode(String mode) {
        if(null == mode || "".equals(mode)) {
            return SegMode.DEFAULT;
        }
        if(mode.equalsIgnoreCase(SegMode.DEFAULT.getValue()) ||
                mode.equalsIgnoreCase(SegMode.INDEX.getValue())) {
            return SegMode.INDEX;
        }
        if(mode.equalsIgnoreCase(SegMode.SEARCH.getValue()) ||
                mode.equalsIgnoreCase(SegMode.QUERY.getValue())) {
            return SegMode.QUERY;
        }
        return SegMode.DEFAULT;
    }
    ...
    public List<SegToken> process(String paragraph) {
        return process(paragraph,SegMode.DEFAULT.DEFAULT.getValue());
    }

    public List<SegToken> process(String paragraph, String mode) {
        if(null == mode || "".equals(mode)) {
            mode = this.mode;
            if(null == mode || "".equals(mode)) {
            ...
            if(null == mode || "".equals(mode)) {
                mode = SegMode.DEFAULT.getValue();
            }
        }
        if(null == mode || "".equals(mode)) {
            return process(paragraph,SegMode.DEFAULT.DEFAULT);
        }
        if(mode.equalsIgnoreCase("default") ||
                mode.equalsIgnoreCase("index")) {
            return process(paragraph,SegMode.INDEX);
        }
        if(mode.equalsIgnoreCase("search") ||
                mode.equalsIgnoreCase("query")) {
            return process(paragraph,SegMode.SEARCH);
        }
        return process(paragraph,SegMode.INDEX);
    }
    ...
    public List<SegToken> process(String paragraph, SegMode mode) {
        if(null == mode) {
            mode = toMode(this.config.mode());
        }
        List<SegToken> tokens = new ArrayList<SegToken>();
        StringBuilder sb = new StringBuilder();
        int offset = 0;
        for (int i = 0; i < paragraph.length(); ++i) {
            char ch = CharacterUtils.regularize(paragraph.charAt(i));
            if (CharacterUtils.ccFind(ch)) {
                sb.append(ch);
            } else {
                if (sb.length() > 0) {
                    if (mode == SegMode.SEARCH || mode == SegMode.QUERY) {
                        for (String word : sentenceProcess(sb.toString())) {
                            //如果是停用词，则跳过
                            if(wordDict.isStopword(word)) {
                                continue;
                            }
                ...
                if (wordDict.containsWord(paragraph.substring(i, i + 1))) {
                    //如果不是停用词，才添加
                    if(!wordDict.isStopword(paragraph)) {
