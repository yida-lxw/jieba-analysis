...
public class FinalSeg {
    ...
    private void loadModel() {
        ...
        String probPath = config.modelPath();
        if(null == probPath || "".equals(probPath)) {
            probPath = PROB_EMIT;
        } else {
            probPath = probPath + PROB_EMIT;
        }
        /*InputStream is = this.getClass().getResourceAsStream(probPath);
        if(null == is) {
            is = this.getClass().getClassLoader().getResourceAsStream(PROB_EMIT);
            if(null == is) {
                throw new RuntimeException(String.format(Locale.getDefault(),
                        "%s: model file{%s} not found!", PROB_EMIT));
            }
        }*/

        File file = FileUtils.makeFile(probPath);
        if(null == file) {
            throw new RuntimeException("load model file from path[" +
                    probPath + "] occur exception,please check out " +
                    "the filePath you input.");
        }
        ...
        try {
            ...
            throw new RuntimeException(String.format(Locale.getDefault(),
                    "%s: load model failure!", PROB_EMIT));
        }
        finally {
            try {
                if (null != reader) {
    ...
    private void processOtherUnknownWords(String other, List<String> tokens) {
        Matcher mat = CharacterUtils.reSkip.matcher(other);
        int offset = 0;
        while (mat.find()) {
            if (mat.start() > offset) {
